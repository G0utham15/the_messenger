...
def before_request():
    ...
    set_lang(lang)
    '''
    if request.path.startswith('/admin') or request.path.startswith('/mod') or request.path.startswith('/biz') or request.path.startswith('/user'):
        if not current_user.is_authenticated:
            flash("You are not authorized", "error")
            return redirect(url_for('security.login'))

@members_blueprint.route('/success')
@login_required
def member_page():
